...
class GaussianNaiveBayes(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        for i, cls in enumerate(self.classes_):
            self.mu_[i] = np.array(X[y == cls].mean(axis=0))
    ...
    def likelihood(self, X: np.ndarray) -> np.ndarray:
        ...
        for i in range(len(self.classes_)):
            diff = X - self.mu_[i]
            normal_factor = 2 * np.pi ** (X.shape[1] / 2) * np.sqrt(np.prod(self.vars_[i]))
            gauss_exp = np.exp(-0.5 * np.sum(np.power(diff, 2) / self.vars_[i], axis=1))
            likelihood[:, i] = (self.pi_[i] * gauss_exp / normal_factor)
